      ...
      <AnimatePresence>
        {entries.map((entry) => (
          ...
          >
            ...
            <ConfirmDeleteModal onConfirm={() => removeEntry(entry.id)}>
              <button className="shrink-0 cursor-pointer rounded-lg border border-red-950/30 bg-red-950/20 px-3 py-1.5 text-xs font-semibold text-red-400/80 transition-all hover:border-red-950/60 hover:bg-red-950/40 hover:text-red-400">
                Delete
              </button>
            </ConfirmDeleteModal>
          </motion.div>
